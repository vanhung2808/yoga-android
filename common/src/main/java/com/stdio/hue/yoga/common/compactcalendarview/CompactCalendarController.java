package com.stdio.hue.yoga.common.compactcalendarview;
...
import com.stdio.hue.yoga.common.compactcalendarview.domain.Event;
...
import static com.stdio.hue.yoga.common.compactcalendarview.CompactCalendarView.FILL_LARGE_INDICATOR;
import static com.stdio.hue.yoga.common.compactcalendarview.CompactCalendarView.NO_FILL_LARGE_INDICATOR;
import static com.stdio.hue.yoga.common.compactcalendarview.CompactCalendarView.SMALL_INDICATOR;
...
class CompactCalendarController {
        ...
        for (int dayColumn = 0, colDirection = isRtl ? 6 : 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                if (currentCalender.get(Calendar.DAY_OF_MONTH) == day && isSameMonthAsCurrentCalendar && !isAnimatingWithExpose) {
                    drawDayRectIndicator(currentSelectedDayIndicatorStyle, canvas, xPosition, yPosition, currentSelectedDayBackgroundColor);
                    ...
                    drawDayRectIndicator(currentDayIndicatorStyle, canvas, xPosition, yPosition, currentDayBackgroundColor);
    ...
    private void drawEventIndicatorCircle(Canvas canvas, float x, float y, int color) {
        ...
        if (eventIndicatorStyle == SMALL_INDICATOR) {
            dayPaint.setStyle(Paint.Style.FILL);
            drawEventCircle(canvas, smallIndicatorRadius, x, y);
        } else if (eventIndicatorStyle == NO_FILL_LARGE_INDICATOR) {
            dayPaint.setStyle(Paint.Style.STROKE);
            drawEventCircleIndicator(NO_FILL_LARGE_INDICATOR, canvas, x, y, color);
        } else if (eventIndicatorStyle == FILL_LARGE_INDICATOR) {
            drawEventCircleIndicator(FILL_LARGE_INDICATOR, canvas, x, y, color);
        }
    }

    private void drawEventCircleIndicator(int indicatorStyle, Canvas canvas, float x, float y, int color) {
        drawEventCircleIndicator(indicatorStyle, canvas, x, y, color, 1);
    }

    private void drawEventCircleIndicator(int indicatorStyle, Canvas canvas, float x, float y, int color, float circleScale) {
        ...
        drawEventCircle(canvas, x, y, color, circleScale);
        dayPaint.setStrokeWidth(strokeWidth);
        dayPaint.setStyle(Paint.Style.FILL);
    }

    private void drawEventCircle(Canvas canvas, float x, float y, int color, float circleScale) {
        dayPaint.setColor(color);
        if (animationStatus == ANIMATE_INDICATORS) {
            float maxRadius = circleScale * bigCircleIndicatorRadius * 1.4f;
            drawEventCircle(canvas, growfactorIndicator > maxRadius ? maxRadius : growfactorIndicator, x, y - (textHeight / 6));
        } else {
            drawEventCircle(canvas, circleScale * bigCircleIndicatorRadius, x, y - (textHeight / 6));
        }
    }

    private void drawEventCircle(Canvas canvas, float radius, float x, float y) {
        canvas.drawCircle(x, y, radius, dayPaint);
    }


    //Draw day
    private void drawDayRectIndicator(int indicatorStyle, Canvas canvas, float x, float y, int color) {
        drawDayRectIndicator(indicatorStyle, canvas, x, y, color, 1);
    }

    private void drawDayRectIndicator(int indicatorStyle, Canvas canvas, float x, float y, int color, float circleScale) {
        float strokeWidth = dayPaint.getStrokeWidth();
        if (indicatorStyle == NO_FILL_LARGE_INDICATOR) {
            dayPaint.setStrokeWidth(2 * screenDensity);
            dayPaint.setStyle(Paint.Style.STROKE);
    ...
    private void drawRect(Canvas canvas, float x, float y) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            canvas.drawRoundRect(x - 45, y - 55, x + 45, y + 30, 15, 15, dayPaint);
        } else {
            canvas.drawRect(x - 45, y - 55, x + 45, y + 30, dayPaint);
        }
    }
